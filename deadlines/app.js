...
function card(e,now){
 ...
 const status=e.estimate?'Estimated · next regular cycle':e.track==='postdeadline'?'Post-deadline · graduate eligible':state==='urgent'?'Due within 7 days':state==='check'?'Check exact cutoff':'Official date';
...
function journalRow(e,papers){const examples=papers.filter(p=>e.paper_ids.includes(p.id));return `<div class="journal-row" id="${esc(e.id)}"><div><a class="journal-name" href="${esc(url(e.website))}" target="_blank" rel="noopener noreferrer">${esc(e.title)}</a><p class="meta">${e.topics.includes('integrated-circuits')?'Design automation / electronics':e.topics.includes('quantum')?'Quantum photonics':'Photonics'} · rolling submissions</p></div><div>${esc(e.fit)}${examples.length?`<details><summary>Recent integrated-photonics papers (${examples.length})</summary>${paperList(examples)}</details>`:''}${e.format.length?`<details><summary>Format notes</summary><ul class="format-list">${e.format.map(f=>`<li>${esc(f)}</li>`).join('')}</ul></details>`:''}</div><div>${external(e.format_source,'Author information')}<p>${external(e.website,'Latest articles')}</p></div></div>`;}
function journalDirectory(entries,papers,data){const tiers={1:'Broad-visibility and field-leading journals',2:'High-visibility specialist and cross-field journals',3:'Established specialist and applied journals'};return `<section id="journal-directory"><h2>Regular journals <small>${entries.length}</small></h2><p class="section-note">No fixed deadline for ordinary research submissions. ${esc(data.catalog.ranking)}</p><p class="section-note">Coverage: integrated photonics, optical links, materials/devices, quantum hardware and design automation. Reference publication history: ${external(data.catalog.lab_source,'Bergman / Columbia Lightwave Lab')}. Review-only and invitation-only outlets are excluded.</p>${Object.entries(tiers).map(([tier,title])=>{const rows=entries.filter(e=>e.influence_tier===Number(tier)).sort((a,b)=>a.title.localeCompare(b.title));return rows.length?`<section class="journal-group"><h3>${tier}. ${title} <span class="meta">(${rows.length})</span></h3><div class="journal-head"><span>Journal</span><span>Fit / examples</span><span>Submission / articles</span></div>${rows.map(e=>journalRow(e,papers)).join('')}</section>`:'';}).join('')}${entries.length?'':'<p class="empty">No matching journals.</p>'}</section>`;}
function meetingPlan(raw,query){const now=new Date(),rows=raw.filter(e=>e.kind==='conference'&&e.track==='regular'&&e.category==='core').flatMap(e=>[e.event?{...e.event,name:e.short_name,cycle:e.cycle}:null,e.announced_next?{...e.announced_next.event,name:e.short_name,cycle:e.announced_next.cycle}:null]).filter(Boolean).filter(e=>Date.parse(e.end+'T23:59:59-12:00')>=now.getTime()).filter(e=>!query||[e.name,e.city,e.venue].join(' ').toLowerCase().includes(query)).sort((a,b)=>a.start.localeCompare(b.start));return `<h2>Announced core-conference meetings</h2><p class="section-note">Meeting dates are independent of submission availability. A meeting can remain upcoming after its regular submission gate has closed.</p><div class="table-wrap"><table><thead><tr><th>Conference</th><th>Dates</th><th>Location / venue</th><th>Source</th></tr></thead><tbody>${rows.map(e=>`<tr><td>${esc(e.name+' '+e.cycle)}</td><td>${esc(dateRange(e))}</td><td>${esc(e.city)}<br><span class="meta">${esc(e.venue)}</span></td><td>${external(e.source,'Official')}</td></tr>`).join('')}</tbody></table></div>`;}
async function start(){
 ...
 try{
  ...
  function render(){const now=new Date(),q=search.value.trim().toLowerCase(),open=new Set([...results.querySelectorAll('details[open]')].filter(e=>e.id).map(e=>e.id));entries=visibleEntries(data.entries,page,now);
   const matches=e=>(area.value==='all'||e.topics.includes(area.value))&&(!q||[e.title,e.short_name,e.fit,e.note,e.event?.city,e.event?.venue,...e.format,...recent.filter(p=>e.paper_ids.includes(p.id)).map(p=>p.title+' '+p.topic)].join(' ').toLowerCase().includes(q));shown=entries.filter(matches);
   if(page==='conferences'){
    results.innerHTML=section('core','Core conferences · regular submissions',shown.filter(e=>e.category==='core'&&e.track==='regular'),now,'OFC · CLEO · IPC · IEEE SiPhotonics · Photonics West · OIP · NUSOD')+section('postdeadline','Post-deadline · graduate-student eligible',shown.filter(e=>e.track==='postdeadline'),now,'Separate calls for significant new results. Eligibility evidence and any student-award restrictions are listed in each entry.')+section('adjacent','Design automation / adjacent conferences',shown.filter(e=>e.category==='adjacent'&&e.track==='regular'),now,'ICCAD · DAC · ASP-DAC · DATE. Check the listed track and contribution fit.');
    ...
    results.innerHTML=section('special','Special-issue deadlines',shown.filter(e=>e.kind==='special-issue'),now,'Active calls, ordered by submission deadline.')+journalDirectory(shown.filter(e=>e.kind==='journal'),recent,data);
    ...
    document.querySelector('#recent-papers').innerHTML=`<h2>Recent integrated-photonics papers <small>${papers.length}</small></h2><p class="section-note">Selected papers from the current and previous calendar year, with publisher / DOI links.</p>${papers.map(p=>`<article class="paper-row"><h3>${external(p.url,p.title)}</h3><p>${esc(data.entries.find(e=>e.id===p.journal_id)?.title)} · ${esc(p.published_on||p.year)}${p.bergman?' · Bergman group':''}</p><p>${esc(p.topic)}</p></article>`).join('')||'<p class="empty">No matching recent papers.</p>'}`;
   }
  ...
  const linked=document.getElementById(location.hash.slice(1));if(linked){if(linked.tagName==='DETAILS')linked.open=true;linked.scrollIntoView();}
