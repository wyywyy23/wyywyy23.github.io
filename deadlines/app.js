...
export function journalCalls(journal,entries,now=new Date()){return entries.filter(e=>e.kind==='special-issue'&&e.journal_id===journal.id&&!isClosed(e,now)).sort(deadlineSort);}
function journalRow(e,papers,calls,now){
 const examples=papers.filter(p=>e.paper_ids.includes(p.id));
 const name=e.short_name!==e.title?e.short_name+' · '+e.title:e.title;
 const activeCalls=journalCalls(e,calls,now),callLinks=activeCalls.length?`<details><summary>Current calls (${activeCalls.length})</summary><ul class="paper-list">${activeCalls.map(c=>`<li><a href="#${esc(c.id)}">${esc(c.title)}</a> <span class="meta">· ${esc(dateText(primaryDeadline(c).date))}</span></li>`).join('')}</ul></details>`:e.submission_mode==='topical'?`<p class="meta">No matching current calls listed.</p>`:'';
 const openCalls=e.links.find(l=>l.label==='Open calls');
 return `<div class="journal-row" id="${esc(e.id)}"><div><a class="journal-name" href="${esc(url(e.website))}" target="_blank" rel="noopener noreferrer">${esc(name)}</a>${e.article_format?`<p class="article-format">${external(e.article_format.source,e.article_format.label)}</p>`:`<p class="meta">${e.submission_mode==='topical'?'Topic-specific deadlines':'Rolling submissions'}</p>`}</div><div>${esc(e.fit)}${examples.length?`<details><summary>Recent papers (${examples.length})</summary>${paperList(examples)}</details>`:''}${e.format.length?`<details><summary>Format requirements</summary><ul class="format-list">${e.format.map(f=>`<li>${esc(f)}</li>`).join('')}</ul></details>`:''}${callLinks}</div><div>${external(e.format_source,'Author information')}<p>${external(e.website,'Latest articles')}</p>${openCalls?`<p>${external(openCalls.url,'All official CFPs')}</p>`:''}</div></div>`;
}
function journalTable(entries,papers,calls,now){return entries.length?'<div class="journal-head"><span>Journal / article type</span><span>Suitable work / papers</span><span>Submission / articles</span></div>'+entries.map(e=>journalRow(e,papers,calls,now)).join(''):'<p class="empty">No matching journals.</p>';}
function journalDirectory(entries,papers,calls,now,expanded){
 ...
 const fold=(id,title,rows,content,defaultOpen=false)=>rows.length?`<details class="target-fold" id="${id}"${expanded||defaultOpen?' open':''}><summary><h2>${title} <small>${rows.length}</small></h2><span aria-hidden="true">+</span></summary>${content}</details>`:'';
 ...
 const specialist=Object.entries(topics).map(([id,title])=>{const rows=g.specialist.filter(e=>e.target_topic===id);return rows.length?`<section class="topic-group" id="topic-${id}"><h3>${title} <span class="meta">${rows.length}</span></h3>${journalTable(rows,papers,calls,now)}</section>`:'';}).join('');
 return `<div id="journal-directory"><section id="core-journals"><h2>Core photonics <small>${g.core.length}</small></h2>${journalTable(g.core,papers,calls,now)}</section><section id="letter-journals"><h2>Letters & short papers <small>${g.letters.length}</small></h2>${journalTable(mainLetters,papers,calls,now)}${entries.some(e=>e.id==='optica')?'<p class="format-route">Also: <a href="#optica">Optica · 4-page Letters</a></p>':''}${fold('other-letters','Physics / nanomaterials letters',otherLetters,journalTable(otherLetters,papers,calls,now))}</section><section id="device-journals"><h2>Devices & platforms <small>${g.devices.length}</small></h2>${journalTable(g.devices,papers,calls,now)}</section>${section('special','Special-issue deadlines',calls,now,'')}${fold('reach-journals','Reach targets',g.reach,journalTable(g.reach,papers,calls,now),true)}${fold('specialist-journals','Other journals by topic',g.specialist,specialist)}</div>`;
}
