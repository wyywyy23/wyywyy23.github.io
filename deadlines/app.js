...
function journalRow(e,papers){const examples=papers.filter(p=>e.paper_ids.includes(p.id));return `<div class="journal-row" id="${esc(e.id)}"><div><a class="journal-name" href="${esc(url(e.website))}" target="_blank" rel="noopener noreferrer">${esc(e.title)}</a><p class="meta">Rolling submissions</p></div><div>${esc(e.fit)}${examples.length?`<details><summary>Recent integrated-photonics papers (${examples.length})</summary>${paperList(examples)}</details>`:''}${e.format.length?`<details><summary>Format notes</summary><ul class="format-list">${e.format.map(f=>`<li>${esc(f)}</li>`).join('')}</ul></details>`:''}</div><div>${external(e.format_source,'Author information')}<p>${external(e.website,'Latest articles')}</p></div></div>`;}
function journalDirectory(entries,papers){return `<section id="journal-directory"><h2>Regular journals <small>${entries.length}</small></h2>${entries.length?'<div class="journal-head"><span>Journal</span><span>Fit / examples</span><span>Submission / articles</span></div>'+entries.map(e=>journalRow(e,papers)).join(''):'<p class="empty">No matching journals.</p>'}</section>`;}
 ...
 try{
  ...
  function render(){const now=new Date(),q=search.value.trim().toLowerCase(),open=new Set([...results.querySelectorAll('details[open]')].filter(e=>e.id).map(e=>e.id));entries=visibleEntries(data.entries,page,now);
   ...
   if(page==='conferences'){
    ...
    results.innerHTML=section('special','Special-issue deadlines',shown.filter(e=>e.kind==='special-issue'),now,'')+journalDirectory(shown.filter(e=>e.kind==='journal'),recent);
